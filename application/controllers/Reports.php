...
class Reports extends CI_Controller
{
    ...
    function profit_loss()
    {
        if ($this->applib->have_access_role(STOCK_STATUS_MODULE_ID, "view") && $this->applib->have_access_role(REPORT_MODULE_ID, 'view')) {
            ...
            $data['gold_rate'] = $this->crud->get_column_value_by_id('settings', 'settings_value', array('settings_key' => 'gold_rate'));
            $data['silver_rate'] = $this->crud->get_column_value_by_id('settings', 'settings_value', array('settings_key' => 'silver_rate'));
            set_page('reports/profit_loss', $data);
        } else {
            $this->session->set_flashdata('error_message', 'You have not permission to access this page.');
            redirect("/");
        }
    ...
    function stock_status_datatable()
    {
        ...
        $post_data = $this->input->post();
        return get_stock_status_datatable($post_data);
    }
    ...
    function profit_loss_datatable()
    {
        ...
        $include_wstg = false;
        $post_data = $this->input->post();
        // print_r($post_data['item_id']).exit;
        ...
        foreach ($list as $stock) {
            ...
            //$row[] = '<b class="' . $className . '">' . $grwt . 'TEST</b>';
            ...
            //$row[] = '<b class="' . $className . '">' . $stock->rfid_created_grwt . 'TT</b>';
            ...
            // $row[] = '<b class="' . $className . '">' . $rfid_not_created_grwt . 'T</b>';
            ...
            $opening_data = $this->get_opening_stock_ledger($from_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], '', $post_data['include_wastage']);
            $p_data = $this->crud->get_sell_items_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'P');
            $s_data = $this->crud->get_sell_items_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'S');
            $e_data = $this->crud->get_sell_items_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'E');
            $m_r_data = $this->crud->get_metal_payment_receipt_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'M R');
            $m_p_data = $this->crud->get_metal_payment_receipt_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'M P');
            $f_t_data = $this->crud->get_stock_transfer_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'F T');
            $t_t_data = $this->crud->get_stock_transfer_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'T T');
            $mfi_data = $this->crud->get_manufacture_issue_receive_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MFI');
            $mfr_data = $this->crud->get_manufacture_issue_receive_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MFR');
            $mfis_data = $this->crud->get_manufacture_issue_receive_silver_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MFIS');
            $mfrs_data = $this->crud->get_manufacture_issue_receive_silver_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MFRS');
            $mhmifw_data = $this->crud->get_manufacture_manu_hand_made_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MHMIFW');
            $mhmis_data = $this->crud->get_manufacture_manu_hand_made_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MHMIS');
            $mhmrfw_data = $this->crud->get_manufacture_manu_hand_made_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MHMRFW');
            $mhmrs_data = $this->crud->get_manufacture_manu_hand_made_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MHMRS');
            $castingifw_data = $this->crud->get_manufacture_casting_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'CASTINGIFW');
            $castingis_data = $this->crud->get_manufacture_casting_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'CASTINGIS');
            $castingrfw_data = $this->crud->get_manufacture_casting_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'CASTINGRFW');
            $castingrs_data = $this->crud->get_manufacture_casting_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'CASTINGRS');
            $mchainifw_data = $this->crud->get_manufacture_machin_chain_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MCHAINIFW');
            $mchainis_data = $this->crud->get_manufacture_machin_chain_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MCHAINIS');
            $mchainrfw_data = $this->crud->get_manufacture_machin_chain_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MCHAINRFW');
            $mchainrs_data = $this->crud->get_manufacture_machin_chain_for_stock_ledger($from_date, $to_date, $department_id, $category_id, $item_id, $profit_loss_tunch, $post_data['account_id'], 'MCHAINRS');
    ...
    function stock_ledger_datatable()
    {
        ...
        foreach ($stock_ledger_data as $key => $stock_ledger) {
            ...
            $row[] = '<b class="' . $className . '">' . $grwt . '</b>';
            $row[] = '<b class="' . $className . '">' . number_format((float) $less, 3, '.', '') . '</b>';
            $row[] = '<b class="' . $className . '">' . $net_wt . '</b>';
            $row[] = '<b class="' . $className . '">' . number_format((float) $touch_id, 2, '.', '') . '</b>';
            $row[] = '<b class="' . $className . '">' . $wstg . '</b>';
            $row[] = '<b class="' . $className . '">' . $fine . '</b>';
            $row[] = '<b class="' . $className . '">' . number_format((float) $balance_grwt, 3, '.', '') . '</b>';
            $row[] = '<b class="' . $className . '">' . number_format((float) $balance_net_wt, 3, '.', '') . '</b>';
            $row[] = '<b class="' . $className . '">' . number_format((float) $balance_fine, 3, '.', '') . '</b>';
        ...
        $row[] = '<b>Total Plus</b>';
        $row[] = '';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_plus_grwt, 3, '.', '') . '</b>';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_plus_less, 3, '.', '') . '</b>';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_plus_net_wt, 3, '.', '') . '</b>';
        ...
        $row[] = ''; //'<b>'.$tunch.'</b>';
        $row[] = '';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_plus_fine, 3, '.', '') . '</b>';
        $row[] = '';
        $row[] = '';
        $row[] = '';
        $data[] = $row;
        ...
        $row[] = '<b>Total Minus</b>';
        $row[] = '';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_minus_grwt, 3, '.', '') . '</b>';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_minus_less, 3, '.', '') . '</b>';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_minus_net_wt, 3, '.', '') . '</b>';
        ...
        $row[] = ''; //'<b>'.$tunch.'</b>';
        $row[] = '';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_minus_fine, 3, '.', '') . '</b>';
        $row[] = '';
        $row[] = '';
        $row[] = '';
        $data[] = $row;
        ...
        $row[] = '<b>Grand Total</b>';
        $row[] = '';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_plus_grwt + (float) $total_minus_grwt, 3, '.', '') . '</b>';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_plus_less + (float) $total_minus_less, 3, '.', '') . '</b>';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_plus_net_wt + (float) $total_minus_net_wt, 3, '.', '') . '</b>';
        ...
        $row[] = '';
        $row[] = '<b class="' . $className . '">' . number_format((float) $total_plus_fine + (float) $total_minus_fine, 3, '.', '') . '</b>';
        $row[] = '';
        $row[] = '';
        $row[] = '';
        $data[] = $row;
        ...
        // Profit Loss Row
        $profit_loss = number_format((float) $balance_fine, '3', '.', '') - (number_format((float) $total_plus_fine, '3', '.', '') + number_format((float) $total_minus_fine, '3', '.', ''));
        $class = ($profit_loss >= 0) ? 'text-primary' : 'text-danger';
        $row = array();
        $row[] = '';
        $row[] = '';
        $row[] = '';
        $row[] = '<b class="' . $className . '">Profit/ Loss</b>';
        $row[] = '';
        $row[] = '';
        $row[] = '';
        $row[] = '';
                    ...
                    if (isset($customer_ledger->journal_id) && !empty($customer_ledger->journal_id)) {
                        ...
                        if ($this->app_model->have_access_role(SELL_PURCHASE_MODULE_ID, "edit")) {
                            if (in_array($customer_ledger->department_id, $department_ids)) {
                                ...
                                $raw_particular_data = $this->get_row_particular_data($post_data['account_id'], $customer_ledger->st_id);
                                $checked = !empty($raw_particular_data) ? 'checked' : '';
                                $action .= '<input type="checkbox" class="myCheckbox" data-id=' . $customer_ledger->st_id . '" ' . $checked . '>';
                            }
                        }
    ...
    function balance_sheet_gold()
    {
        if ($this->applib->have_access_role(REPORT_MODULE_ID, "view") && $this->applib->have_access_role(BALANCE_SHEET_MODULE_ID, 'view')) {
            ...
            $data['gold_rate'] = $this->crud->get_column_value_by_id('settings', 'settings_value', array('settings_key' => 'gold_rate'));
            $data['silver_rate'] = $this->crud->get_column_value_by_id('settings', 'settings_value', array('settings_key' => 'silver_rate'));
            set_page('reports/balance_sheet_gold', $data);
        } else {
            $this->session->set_flashdata('error_message', 'You have not permission to access this page.');
            redirect("/");
        }
    ...
    function balance_sheet_gold_datatable()
    {
        ...
        foreach ($list as $key => $list_row) {
            ...
            if (in_array($account_id, $account_id_arr) && !empty($outstanding_data_arr[$account_id])) {
                ...
                $gold_amount = (float) $outstanding_data_arr[$account_id]['gold_fine'] * (float) $gold_rate;
                $silver_amount = (float) $outstanding_data_arr[$account_id]['silver_fine'] * (float) $silver_rate / 10;
                if ($account_group_id_department == 1) { // Use only Cash Amount of Department
                    $net_amount = (float) $outstanding_data_arr[$account_id]['amount'];
                } else {
                ...
                $gold_amount = (float) $list_row->gold_fine * (float) $gold_rate;
                $silver_amount = (float) $list_row->silver_fine * (float) $silver_rate / 10;
                if ($account_group_id_department == 1) { // Use only Cash Amount of Department
                    $net_amount = (float) $list_row->amount;
                } else {
    ...
    function trading_pl_gold()
    {
        $data = array();
        if ($this->applib->have_access_role(TRADING_PL_MODULE_ID, "view")) {
            set_page('reports/trading_pl_gold', $data);
        } else {
            redirect("/");
        }
    }
    ...
    function trading_pl_gold_datatable()
    {
        ...
        if (!empty($sell_pre_data)) {
            foreach ($sell_pre_data as $sell) {
                if ($sell->type == 1) {
                    if ($sell->category_group_id == CATEGORY_GROUP_GOLD_ID) {
                        $pre_sell_amt = $pre_sell_amt + ($sell->fine);
                    } else if ($sell->category_group_id == CATEGORY_GROUP_SILVER_ID) {
                        $pre_sell_amt = $pre_sell_amt + ($sell->fine * $silver_rate);
                        $pre_sell_amt = $pre_sell_amt / $gold_rate * 10;
                    }
                } else if ($sell->type == 2 || $sell->type == 3) {
                    if ($sell->category_group_id == CATEGORY_GROUP_GOLD_ID) {
                        $pre_purchase_ex_amt = $pre_purchase_ex_amt + ($sell->fine);
                    } else if ($sell->category_group_id == CATEGORY_GROUP_SILVER_ID) {
                        $pre_purchase_ex_amt = $pre_purchase_ex_amt + ($sell->fine * $silver_rate);
                        $pre_purchase_ex_amt = $pre_purchase_ex_amt / $gold_rate * 10;
                    }
                }
            }
        }
        ...
        if (!empty($metal_pre_data)) {
            foreach ($metal_pre_data as $metal) {
                if ($metal->metal_payment_receipt == 1) {
                    if ($metal->category_group_id == CATEGORY_GROUP_GOLD_ID) {
                        $pre_metal_pay_amt = $pre_metal_pay_amt + ($metal->metal_fine);
                    } else if ($metal->category_group_id == CATEGORY_GROUP_SILVER_ID) {
                        $pre_metal_pay_amt = $pre_metal_pay_amt + ($metal->metal_fine * $silver_rate);
                        $pre_metal_pay_amt = $pre_metal_pay_amt / $gold_rate * 10;
                    }
                } else if ($metal->metal_payment_receipt == 2) {
                    if ($metal->category_group_id == CATEGORY_GROUP_GOLD_ID) {
                        $pre_metal_re_amt = $pre_metal_re_amt + ($metal->metal_fine);
                    } else if ($metal->category_group_id == CATEGORY_GROUP_SILVER_ID) {
                        $pre_metal_re_amt = $pre_metal_re_amt + ($metal->metal_fine * $silver_rate);
                        $pre_metal_re_amt = $pre_metal_re_amt / $gold_rate * 10;
                    }
                }
            }
        }
        ...
        //gold amt = fine * gold rate ; //
        $sell_data = $this->crud->getFromSQL("SELECT si.*,c.category_group_id FROM sell as s LEFT JOIN sell_items as si ON si.sell_id = s.sell_id LEFT JOIN category as c ON c.category_id = si.category_id WHERE s.sell_date >= '" . $from_date . "' AND s.sell_date <= '" . $to_date . "' ");
        if (!empty($sell_data)) {
            foreach ($sell_data as $sell) {
                if ($sell->type == 1) {
                    if ($sell->category_group_id == CATEGORY_GROUP_GOLD_ID) {
                        $sell_amt = $sell_amt + ($sell->fine);
                    } else if ($sell->category_group_id == CATEGORY_GROUP_SILVER_ID) {
                        $sell_amt = $sell_amt + ($sell->fine * $silver_rate);
                        $sell_amt = $sell_amt / $gold_rate * 10;
                    }
                } else if ($sell->type == 2 || $sell->type == 3) {
                    if ($sell->category_group_id == CATEGORY_GROUP_GOLD_ID) {
                        $purchase_ex_amt = $purchase_ex_amt + ($sell->fine);
                    } else if ($sell->category_group_id == CATEGORY_GROUP_SILVER_ID) {
                        $purchase_ex_amt = $purchase_ex_amt + ($sell->fine * $silver_rate);
                        $purchase_ex_amt = $purchase_ex_amt / $gold_rate * 10;
                    }
                }
            }
        }
        ...
        if (!empty($metal_data)) {
            foreach ($metal_data as $metal) {
                if ($metal->metal_payment_receipt == 1) {
                    if ($metal->category_group_id == CATEGORY_GROUP_GOLD_ID) {
                        $metal_pay_amt = $metal_pay_amt + ($metal->metal_fine);
                    } else if ($metal->category_group_id == CATEGORY_GROUP_SILVER_ID) {
                        $metal_pay_amt = $metal_pay_amt + ($metal->metal_fine * $silver_rate);
                        $metal_pay_amt = $metal_pay_amt / $gold_rate * 10;
                    }
                } else if ($metal->metal_payment_receipt == 2) {
                    if ($metal->category_group_id == CATEGORY_GROUP_GOLD_ID) {
                        $metal_re_amt = $metal_re_amt + ($metal->metal_fine);
                    } else if ($metal->category_group_id == CATEGORY_GROUP_SILVER_ID) {
                        $metal_re_amt = $metal_re_amt + ($metal->metal_fine * $silver_rate);
                        $metal_re_amt = $metal_re_amt / $gold_rate * 10;
                    }
                }
            }
        }
    ...
    function save_account_selected_rows()
    {
        $data = $this->input->post();
        $account_id = isset($data['account_id']) ? $data['account_id'] : null;
        $selected_rows = isset($data['selected_rows']) ? $data['selected_rows'] : [];

        if ($account_id && !empty($selected_rows)) {

            $data = [];
            foreach ($selected_rows as $row_particular) {
                $data[] = [
                    'account_id' => $account_id,
                    'row_particular' => $row_particular
                ];
            }
            $this->db->insert_batch('selected_rows', $data);

            // Return a success message
            echo json_encode(['status' => 'success', 'message' => 'Rows saved successfully']);
        } else {
            // Return error if account_id or selected rows are not provided
            echo json_encode(['status' => 'error', 'message' => 'Invalid account or no rows selected']);
        }
    }

    // Method to get row_particular data for a given account_id
    public function get_row_particular_data($account_id, $st_id)
    {
        $this->db->select('row_particular');
        $this->db->from('selected_rows');
    ...
    function remove_unchecked_rows()
    {
        $data = $this->input->post();
        $account_id = isset($data['account_id']) ? $data['account_id'] : null;
        $selected_rows = isset($data['remove_Id']) ? $data['remove_Id'] : null;

        $this->db->where('account_id', $account_id);
        $this->db->where('row_particular', $selected_rows);
        $this->db->delete('selected_rows');
