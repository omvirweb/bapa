...
class Sell extends CI_Controller
{
    ...
    function get_stock_item_data()
    {
        $item_data = $this->crud->get_all_records(' item_stock', 'item_stock_id', '');
        print json_encode($item_data);
        exit;       
    }
    ...
    function get_account_old_balance($account_id)
    {
        ...
        if (!empty($account_id)) {
            ...
            if (!empty($account_data)) {
                ...
                $data['account_status'] = $account_data[0]->account_status;
            }
        }
        print json_encode($data);
        exit;
...
//                    $pts_items->total_less = $pts_items->total_less - (float) $sell_item->less;
                ...
                if (in_array($sell_item->sell_item_id, $old_purchase_sell_item_ids)) {
                } else {
                    $pts_items->total_grwt = 0;
                    $pts_items->total_less = 0;
                }
                if (in_array($sell_item->sell_item_id, $old_stock_transfer_detail_purchase_sell_item_ids)) {
                } else {
                    $ptst_items->total_grwt = 0;
                    $ptst_items->total_less = 0;
                }
                if (in_array($sell_item->sell_item_id, $old_issue_receive_purchase_sell_item_ids)) {
                } else {
                    $ptir_items->total_grwt = 0;
                    $ptir_items->total_less = 0;
                }
                if (in_array($sell_item->sell_item_id, $old_manu_hand_made_purchase_sell_item_ids)) {
                } else {
                    $ptmhm_items->total_grwt = 0;
                    $ptmhm_items->total_less = 0;
                }
                if (in_array($sell_item->sell_item_id, $old_machine_chain_purchase_sell_item_ids)) {
                } else {
                    $ptmc_items->total_grwt = 0;
                    $ptmc_items->total_less = 0;
                }
    ...
    function update_stock_item_on_sale(){
