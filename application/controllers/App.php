 ...
 */
    ...
    function get_select2_data($table_name, $id_column, $text_column, $search, $page = 1, $where = array(), $order_by=1){
...
//        echo '<pre>'.$this->db->last_query(); exit;
    ...
    function party_name_with_number_select2_source($is_active = ''){
        ...
        $this->db->select("a.account_id,a.account_name,a.account_mobile,a.account_status",$search,$page);
        $this->db->from("account a");
        if(!empty($account_group_ids)){
            $a_g_ids = array();
            if(in_array(CUSTOMER_GROUP, $account_group_ids)){
...
//        echo '<pre>'.$this->db->last_query(); exit;
    ...
    function design_select2_source(){
		$search = isset($_GET['q']) ? $_GET['q'] : '';
		$page = isset($_GET['page']) ? $_GET['page'] : 1;
		$results = array(
			"results" => $this->get_select2_data('design_master', 'design_id', 'design_no', $search, $page),
			"total_count" => $this->count_select2_data('design_master', 'design_id', 'design_no', $search),
		);
		echo json_encode($results);
		exit();
    }
    ...
    function set_master_design_select2_val_by_id($id){
            $this->get_select2_text_by_id('design_master', 'design_id', 'design_no', $id);
    }
...
//        echo $this->db->last_query();
    ...
    function order_status_select2_source(){
        ...
        $where = '';
        $results = array(
                "results" => $this->get_select2_data('order_status', 'order_status_id', 'status', $search, $page,$where,2),
                "total_count" => $this->count_select2_data('order_status', 'order_status_id', 'status', $search),
        );
        echo json_encode($results);
        exit();
	}
    function order_status2_select2_source(){
        $search = isset($_GET['q']) ? $_GET['q'] : '';
        $page = isset($_GET['page']) ? $_GET['page'] : 1;
        $where = '';
        $results = array(
                "results" => $this->get_select2_data('order_status2', 'order_status_id', 'status', $search, $page,$where,2),
                "total_count" => $this->count_select2_data('order_status2', 'order_status_id', 'status', $search),
        );
        echo json_encode($results);
        exit();
	}
    ...
    function set_order_status2_select2_val_by_id($id){
        $this->get_select2_text_by_id('order_status2', 'order_status_id', 'status', $id);
    }
