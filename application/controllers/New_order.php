...
//                        echo '<pre>'; print_r($insert_item); exit;
    ...
    function new_order_list_2() {
        if($this->applib->have_access_role(ORDER_MODULE_ID,"view")) {
            ...
            $department_ids = $this->applib->current_user_order_department_ids();
            $data['department_ids'] = json_encode($department_ids);
            set_page('new_order/new_order_list_2', $data);
        } else {
            $this->session->set_flashdata('error_message', 'You have not permission to access this page.');
            redirect("/");
        }
    ...
    function new_order_datatable() {
        ...
        $config['select'] = 'o.*,a.account_name,s.account_name as supplier_name,a.account_mobile,aa.account_name AS process_name,o.total_weight,o.total_pcs,os.status,os2.status AS status2,IF(o.order_type = 1,"Order","Inquiry") AS type';

        $config['joins'][] = array('join_table' => 'order_status os', 'join_by' => 'os.order_status_id = o.order_status_id', 'join_type' => 'left');

        $config['joins'][] = array('join_table' => 'order_status2 os2', 'join_by' => 'os2.order_status_id = o.order_status2_id', 'join_type' => 'left');
        ...
        $config['column_search'] = array('a.account_name', 'a.account_mobile', 'aa.account_name', 'os2.status','o.order_no', 'DATE_FORMAT(o.order_date,"%d-%m-%Y")', 'DATE_FORMAT(o.delivery_date,"%d-%m-%Y")','s.account_name','DATE_FORMAT(o.supplier_delivery_date,"%d-%m-%Y")','o.total_weight','o.total_pcs','os.status','IF(o.order_type = 1,"Order","Inquiry")');
        
        $config['column_order'] = array(null, 'a.account_name', 'aa.account_name', 'os2.status','o.order_no', 'o.order_date', 'o.delivery_date','o.real_delivery_date','s.account_name','o.supplier_delivery_date','o.total_weight', 'o.total_pcs','os.status', 'IF(o.order_type = 1,"Order","Inquiry")');
        ...
        if(!empty($post_data['order_status3_id'])){
            $config['wheres'][] = array('column_name' => 'o.order_status2_id', 'column_value' => $post_data['order_status3_id']);
        }
        ...
        if (isset($post_data['everything_delivery_from_start']) && $post_data['everything_delivery_from_start'] != 'true'){
            if(!empty($post_data['delivery_from_date']) && strtotime($post_data['delivery_from_date']) > 0){
                $config['wheres'][] = array('column_name' => 'o.delivery_date >=', 'column_value' => date("Y-m-d",strtotime($post_data['delivery_from_date'])));
            }
        }
        if(!empty($post_data['delivery_to_date']) && strtotime($post_data['delivery_to_date']) > 0){
            $config['wheres'][] = array('column_name' => 'o.delivery_date <=', 'column_value' => date("Y-m-d",strtotime($post_data['delivery_to_date'])));
        }
...
//        $entry_department_ids = $this->applib->current_user_department_ids();
        ...
        foreach ($list as $orders) {
            $qr="SELECT count(*) as count FROM `order_lot_item` WHERE order_id = '$orders->order_id'";
            $qty=$this->crud->getFromSQLArray($qr);

            $row = array();
            $action = '';
//            if(in_array($orders->process_id, $entry_department_ids)){
                if($role_edit){
                    $action .= '<input type="checkbox" name="change_order_status[]" id="" class="change_casting_status chk_change_order_status" value="'.$orders->order_id.'" style="margin-right:8px"><a href="' . base_url("new_order/add/" . $orders->order_id) . '"><span class="edit_button glyphicon glyphicon-edit data-href="#"" style="color : #419bf4" >&nbsp;</span></a>';
                }
                if($role_delete){
...
//            }
            ...
            $row[] = $orders->status2;
            $row[] = '<a href="javascript:void(0);" class="item_row"  data-order_id="' . $orders->order_id . '" >' . $orders->order_no . ' ('.$qty[0]['count'].') </a>';
            ...
            $row[] = $orders->type;
            $data[] = $row;
            $pcs_total+=intval($orders->total_pcs);
            $weight_total+=floatval($orders->total_weight);
        }
        $output = array(
            "draw" => $_POST['draw'],
    ...
    function new_order_datatable_totals() {
        ...
        $data_totals=['pcstotal'=>$pcs_total,'weighttotal'=>number_format($weight_total, 3, '.', '')];
       
        echo json_encode($data_totals);
    }
    
    function new_order_datatable_2() {
        $post_data = $this->input->post();
        ...
        if (!empty($post_data['everything_from_start']) && $post_data['everything_from_start'] != 'true'){
            if(!empty($post_data['from_date']) && strtotime($post_data['from_date']) > 0){
                $config['wheres'][] = array('column_name' => 'o.order_date >=', 'column_value' => date("Y-m-d",strtotime($post_data['from_date'])));
            }
        }
        ...
        if (!empty($post_data['everything_delivery_from_start']) && $post_data['everything_delivery_from_start'] != 'true'){
            if(!empty($post_data['delivery_from_date']) && strtotime($post_data['delivery_from_date']) > 0){
                $config['wheres'][] = array('column_name' => 'o.delivery_date >=', 'column_value' => date("Y-m-d",strtotime($post_data['delivery_from_date'])));
            }
        }
...
//            }
            ...
            $row[] = $orders->account_name. ' - ' .$orders->account_mobile;
            $row[] = $orders->process_name;
            $row[] = '<a href="javascript:void(0);" class="item_row" data-order_id="' . $orders->order_id . '" data-qr="'.$qr.'" >' . $orders->order_no . ' ('.$qty[0]['count'].')</a>';
            $row[] = (!empty(strtotime($orders->order_date))) ? date('d-m-Y', strtotime($orders->order_date)) : '';
            $row[] = (!empty(strtotime($orders->delivery_date))) ? date('d-m-Y', strtotime($orders->delivery_date)) : '';
            $row[] = (!empty(strtotime($orders->real_delivery_date))) ? date('d-m-Y', strtotime($orders->real_delivery_date)) : '';
            $row[] = '   ';
...
//        }
        ...
        foreach ($list as $lot_detail) {
            $row = array();
            $action ='<span style="text-align: center;"><input type="checkbox" name="change_orderItem_status" data-lotIeamID="'.$lot_detail->order_lot_item_id.'" value="'.$lot_detail->order_lot_item_id.'"  class="change_orderItem_status"></span>&nbsp;&nbsp;&nbsp;';
            $action .= '<a href="' . base_url("new_order/order_list_print/" . $lot_detail->order_lot_item_id) . '" target="_blank" title="Order List Print" alt="Call Slip Print"><span class="glyphicon glyphicon-print" style="color : #419bf4">&nbsp</a>';   
    ...
    function change_OrderItem_status() {
        
        $data['status'] = $_POST['status'];
        $data['lot_item_id'] = $_POST['lot_item_id'];
        $data['updated_at'] = $this->now_time;
        $data['updated_by'] = $this->logged_in_id;

        $qry="update order_lot_item set item_status_id=".$data['status'].", updated_at='". $data['updated_at'] ."', updated_by='". $data['updated_by'] ."' where order_lot_item_id in (".implode(',',$data['lot_item_id']).")";
        $result=$this->crud->execuetSQL($qry);
...
//            $action .= '<a href="javascript:void(0);" status="1" class="btn btn-primary btn-xs change_item_status" style="margin-bottom: 3px;">Not Started</a>';
    ...
    function change_order_status() {
        $post_data = $this->input->post();
        $orderids=explode(',',$post_data['selectedorderids']);
        foreach($orderids as $orderid){
            $this->crud->update('new_order',array('order_status2_id' => $post_data['order_status2_id']), array('order_id' => $orderid));
        }
