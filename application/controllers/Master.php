...
class Master extends CI_Controller
{
    ...
    function save_opening_stock()
    {
        ...
        if (isset($opening_stock_id) && !empty($opening_stock_id)) {
            ...
            unset($post_data['wstg']); 
            unset($update_data['wstg']);
    ...
    function item_master($item_id = '')
    {
        ...
        if (isset($item_id) && !empty($item_id)) {
            if ($this->applib->have_access_role(ITEM_MASTER_MODULE_ID, "edit")) {
                ...
                $party_item_details = $this->crud->get_row_by_id('item_bom', array('item_id' => $item_id));
                $lineitems = [];
                foreach ($party_item_details as $lot_item) {
                    $oreder_lot_item->design_name = $this->crud->get_column_value_by_id('design_master', 'design_no', array('design_id' => $lot_item->design_id));
                    $oreder_lot_item->design_id = $lot_item->design_id;
                    $oreder_lot_item->qty = $lot_item->qty;
                    $oreder_lot_item->weight = $lot_item->weight;
                    $oreder_lot_item->weight_for_bom = $lot_item->weight_for_bom;
                    $lineitems[] = json_encode($oreder_lot_item);
                }
                $data['party_item_detail'] = sizeof($lineitems) > 0 ? implode(',', $lineitems) : '';


                //echo '<pre>'; print_r($lineitems); exit;
...
//        echo '<pre>'; print_r($_FILES['image']); exit;
        ...
        $line_items_data = isset($post_data['line_items_data']) ? json_decode($post_data['line_items_data']) : [];
        $line_items_index = isset($post_data['line_items_index']) ? $post_data['line_items_index'] : '';
        ...
        $item_id = isset($post_data['item_id']) && !empty($post_data['item_id']) ? $post_data['item_id'] : $result;

        $where = array("item_id" => $item_id);
        $this->crud->delete("item_bom", $where);

        if (sizeof($line_items_data) > 0) {

            foreach ($line_items_data as $temp) {
                $update_user = [
                    'item_id' => $item_id,
                    'qty' => $temp->qty,
                    'weight' => $temp->weight,
                    'weight_for_bom' => $temp->weight_for_bom,
                ...
                $this->crud->insert('item_bom', $update_user);
            }
        }
    ...
    function item_master_datatable()
    {
        ...
        foreach ($list as $item_master) {
            ...
            if ($role_delete) {
                if (in_array($item_master->item_id, array(GOLD_FINE_ITEM_ID, SILVER_FINE_ITEM_ID))) {
                } else {
...
//            echo '<pre>'; print_r($data['modules_roles']); exit;
    ...
    function get_design_master_weight($design_id)
    {
        $data = array();
        $weight_for_bom = $this->crud->get_column_value_by_id('design_master', 'weight_for_bom', array('design_id' => $design_id));
        $data['weight_for_bom'] = $weight_for_bom ? $weight_for_bom : '';
        echo json_encode($data);
        exit;
    }
}
