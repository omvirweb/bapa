    ...
    function get_sell_items_for_outstanding_report($upto_balance_date, $account_group_id)
    {
        ...
        if (is_array($account_group_id)) {
            if (in_array(0, $account_group_id)) {
                ...
                $this->db->where_in('a.account_group_id', $account_group_id);
            }
        } else {
    ...
    function get_account_ids_from_account_group_id($account_group_id)
    {
        ...
        if (is_array($account_group_id)) {
            if (in_array(0, $account_group_id)) {
                ...
                $group_ids_string = implode(',', $account_group_id);
                // $this->db->where_in('a.account_group_id', $account_group_id);
                $sql .= 'WHERE account_group_id IN (' . $group_ids_string . ')';
            }
        } else {
            if (!empty($account_group_id)) {
                // $this->db->where('a.account_group_id', $account_group_id);
                $sql .= 'WHERE account_group_id = ' . $account_group_id;
            }
        }
        $account_arr = $this->getFromSQL($sql);
