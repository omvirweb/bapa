...
<div class="content-wrapper">
    <section class="content-header">
        <h1>
            Stock Ledger <a
                href="<?= base_url('reports/stock_ledger/' . $item_stock_data->item_stock_id . '/' . $item_wise) ?>"
                class="btn btn-primary btn-xs" style="margin: 5px;"><i class="fa fa-refresh"></i></a>
            <a href="<?= base_url('reports/stock_status') ?>" class="btn btn-primary pull-right btn-sm"
                style="margin: 5px;">Stock Status</a>
        </h1>
    </section>
    <style>
        .dataTables_info {
                                    ...
                                    <div class="box-body table-responsive">
                                        ...
                                        <div class="col-md-2">
                                            ...
                                            <select class="form-control select2" id="category_id">
                                                <option value="0" <?= $category_id == '0' ? 'selected' : ''; ?>>All
                                                </option>
                                                <?php foreach ($category as $value) { ?>
                                                    <option value="<?= $value->category_id; ?>"
                                                        <?= $category_id == $value->category_id ? 'selected' : ''; ?>>
                                                        <?= $value->category_name; ?>
                                                    </option>
                                                <?php } ?>
                                            </select>
                                        ...
                                        <div class="col-md-2">
                                            ...
                                            <select id="item_id" class="form-control select2">
                                                <option value="0" <?= $item_id == '0' ? 'selected' : ''; ?>>All</option>
                                                <?php foreach ($items as $value) { ?>
                                                    <option value="<?= $value->item_id; ?>" <?= $item_id == $value->item_id ? 'selected' : ''; ?>>
                                                        <?= $value->item_name; ?>
                                                    </option>
                                                <?php } ?>
                                            </select>
                                            ...
                                            <select id="tunch" class="form-control select2">
                                                ...
                                                foreach ($carat as $value) {
                                                    if ($tunch == $value->purity) {
                                                        echo '<option value="' . $value->purity . '" selected>' . $value->purity . '</option>';
                                                        $tunchFound = true;
                                                    } else {
                                        ...
                                        <div class="col-md-2">
                                            ...
                                            <select name="rfid_filter" class="form-control select2" id="rfid_filter">
                                                <option value="0" <?= $rfid_filter == '0' ? 'selected' : ''; ?>>All
                                                </option>
                                                <option value="1" <?= $rfid_filter == '1' ? 'selected' : ''; ?>>Only RFID
                                                    Stock</option>
                                                <option value="2" <?= $rfid_filter == '2' ? 'selected' : ''; ?>>Without
                                                    RFID Stock</option>
                                            </select>
                                        </div>
                                        <div class="col-md-3">
                                        ...
                                        <!-- <div class="col-md-1">
                                            <br />
                                            <button name="search" id="search" class="btn btn-primary btn-sm pull-left"
                                                autocomplete="off"><span class="fa fa-search-plus"></span>
                                                Search</button>
                                        </div> -->
                                        <div class="col-md-4">
                                            <label class="text-right text-danger pull-right"><br />If Tunch is Not from
                                                Master : Balance Will Not Come</label><br />
                                        </div>
                                        <div class="clearfix"></div><br />
                                        <table class="table row-border table-bordered table-striped" style="width:100%"
                                            id="stock_ledger_table">
                                            <thead>
                                                <tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#datepicker1, #datepicker2, #department_id, #category_id, #item_id, #tunch, #account_id, #type_sort, #rfid_filter, #include_wastage').on('change', function () {
            redrawTable();
        });

        // Optional: if you want to include manual typing triggers (e.g., for date inputs)
        $('#datepicker1, #datepicker2').on('input', function () {
            redrawTable();
        });
