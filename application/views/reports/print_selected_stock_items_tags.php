    ...
    <?php if (!empty($rfid_data)) { ?>
        <table style="font-size: 10px; font-family: Arial Rounded MT Bold;" class="border">
            <?php foreach ($rfid_data as $rfid) {
                ...
                // echo '</pre>';
                // exit;
                $item_stock_rfid = $rfid['item_stock_rfid'];
                $item_master = $rfid['item_master'];
            ?>
                <tr>
                    ...
                    <td>
                        <table style="text-align: right; width:100%">
                            <tr>
                                <td>
                                    <b>Gw: <?php echo number_format($item_stock_rfid->grwt, 3, '.', ''); ?></b><br>
                                    <b>L: <?php echo number_format($item_stock_rfid->less, 3, '.', ''); ?></b><br>
                                    <b>N: <?php echo number_format($item_stock_rfid->ntwt, 3, '.', ''); ?></b>
                                </td>
                                <?php if ($use_barcode == '1') { ?>
                                    <td>
                                        <img alt="barcode"
                                            src="<?= base_url() . '/barcode.php'; ?>?codetype=Code128&size=40&text=<?php echo $item_stock_rfid->item_stock_id; ?>"
