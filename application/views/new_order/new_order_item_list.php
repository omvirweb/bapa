                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="box-body table-responsive">
                                        ...
                                        <a href="javascript:void(0);" status="1" class="btn btn-primary btn-xs change_item_status" style="margin-bottom: 3px;">Change Order Item Status</a> 
    ...
    <div class="modal-dialog">
        <div class="modal-content" >
            <div class="modal-header">
                ...
                <h4 class="modal-title">Order Item Status</h4>
            </div>
            <div class="modal-body">
                <div class="col-md-8">
                    <label for="">Status</label>
                    <select name="" id="Item_status_id" class="form-control">
                ...
                <div class="col-md-8">
                    <br/>
                <button name="button" id="saveChangeOrderItem" class="btn btn-primary btn-sm"><span class=""></span> Save</button>
...
//            table.draw();
        ...
        $(document).on('click', '#saveChangeOrderItem', function(){
            let status=$('#Item_status_id').val()
            let lot_item_id = []
            $("input:checkbox[name=change_orderItem_status]:checked").each(function(){
                lot_item_id.push($(this).val());
            });
            console.log('Hello...', status, lot_item_id);

            if(lot_item_id.length === 0){
                console.log("we don't have data")
                $('#error').html('<p class="error">No DATA SELECTED</p>')
                
            }else{
                $.ajax({
                    url:"<?php echo base_url('new_order/change_OrderItem_status'); ?>",
                    type:'post',
                    data:{status:status,  lot_item_id:lot_item_id},
                    success: function(response){
                        console.log(response, 'response')
                        if(response.data==null){
                            console.log('success')
                            $('#change_item_status_modal').modal('hide');
                            $('#successMessage').html('<h2 class="Success">Bulk Status Succssfull</h2>')
