                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="box-body table-responsive">
                                        ...
                                        <div class="col-md-2">
                                            <label for="remark">Status2</label>    
                                            <select name="order_status3_id" id="order_status3_id" class="form-control order_status_id"></select>
                                        </div>
                                        <div class="col-md-2">
                                        ...
                                        <div class="clearfix"></div><br />
                                        <a href="javascript:void(0);" status="1" class="btn btn-primary btn-xs change_order_status" style="margin-bottom: 3px;">Change Order Status</a>
                                        <table id="lot_master_table" class="table row-border table-bordered table-striped" style="width:100%">
                                            ...
                                            <tr>
                                                ...
                                                <th></th>
                                                <th></th>
                                                <th><span id="weighttotal"></span></th>
                                                <th><span id="pcstotal"></span></th>
                                                <th></th>
                                                <th></th>
                                            </tr>
                                        </tfoot>
...
<div id="change_order_status_modal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content" >
            <form id="frmchangeorderstatus" method="POST" action="">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            ...
            <div class="modal-body">
                <div class="col-md-8">
                    ...
                    <select name="order_status2_id" id="order_status2_id" class="form-control">
...
<script>
    ...
    $(document).ready(function () {
        ...
        initAjaxSelect2($("#order_status2_id"), "<?= base_url('app/order_status2_select2_source') ?>");
        initAjaxSelect2($("#order_status3_id"), "<?= base_url('app/order_status2_select2_source') ?>");
        setSelect2Value($("#order_status_id"), "<?= base_url('app/set_order_status_select2_val_by_id/' . PENDING_STATUS) ?>");
        setSelect2Value($("#order_status2_id"), "<?= base_url('app/set_order_status2_select2_val_by_id/1') ?>");
        setSelect2Value($("#order_status3_id"), "<?= base_url('app/set_order_status2_select2_val_by_id/1') ?>");
...
//            "paging": false,
            ...
            "ajax": {
                ...
                "data": function (d) {
                    ...
                    d.order_status3_id = $('#order_status3_id').val();
...
//                        console.log(json);
        ...
        $(document).on('change', '#order_status3_id', function(){
            var order_status_id = $('#order_status3_id').val();
            if(order_status_id == '' || order_status_id === null){
                $('#select2-order_status3_id-container .select2-selection__placeholder').html(' All ');
            }
            $('#ajax-loader').show();
            table.draw();
        });
        ...
        $(document).on("click", ".change_order_status", function () {
            $('#order_status2_id').val(1).trigger('change');
            // var selectedorderids=[];
            // $('#selectedorderids').val('');
            // $('.chk_change_order_status').each(function(item){
            //     if($(this).is(':checked')){
            //         selectedorderids.push($(this).val());
            //     }
            // })
            // $('#selectedorderids').val(selectedorderids.join(','));
            $('#change_order_status_modal').modal('show');
        });
        $(document).on('click','.chk_change_order_status',function(){
            
            if($(this).is(':checked')){
                
                if(selectedorderids.indexOf($(this).val())==-1){
                   // alert($(this).val()+' hello notfound');
                    selectedorderids.push($(this).val());
                }
            }else{
               // alert('hello unchecked');
                if(selectedorderids.indexOf($(this).val())!=-1){
                    selectedorderids.splice(selectedorderids.indexOf($(this).val()),1);
                }
            }
            $('#selectedorderids').val(selectedorderids.join(','));
        });
        $('#frmchangeorderstatus').submit(function(){
            var dt= $('#frmchangeorderstatus').serialize();
            $.post('<?php echo base_url('new_order/change_order_status'); ?>',dt,function(data){
                $('#change_order_status_modal').modal('hide');
                selectedorderids=[];
                $('#selectedorderids').val('');
                table.draw();
